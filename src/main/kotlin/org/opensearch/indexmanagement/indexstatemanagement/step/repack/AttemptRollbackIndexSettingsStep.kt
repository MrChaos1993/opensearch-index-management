...
class AttemptRollbackIndexSettingsStep : Step(NAME) {
    ...
    private suspend fun setIndexSettings(indexName: String, context: StepContext): Boolean {
        ...
        settingsBuilder.put(
            TieredMergePolicyProvider.INDEX_MERGE_POLICY_EXPUNGE_DELETES_ALLOWED_SETTING.key,
            info[PRE_REPACK_EXPUNGE_DELETES_ALLOWED] as Double,
        )
        try {
            val updateSettingsRequest = UpdateSettingsRequest().indices(indexName).settings(settingsBuilder.build())
            val response: AcknowledgedResponse = context.client.admin().indices().suspendUntil { updateSettings(updateSettingsRequest, it) }
