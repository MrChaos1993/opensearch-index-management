...
class AttemptCreateRepackJobStep : Step(NAME) {
    ...
    // steps:
    // 1. send ForceMergeRequest with onlyExpungeDeletes=true
    ...
    override suspend fun execute(): Step {
        ...
        try {
            ...
            val request = ForceMergeRequest(indexName).onlyExpungeDeletes(true)
            var response: ForceMergeResponse? = null
            var throwable: Throwable? = null
            // I Guess I don't need this coroutine!
            GlobalScope.launch(Dispatchers.IO + CoroutineName("ISM-Repack-$indexName")) {
                try {
                    logger.info("Sending forceMerge requests")
