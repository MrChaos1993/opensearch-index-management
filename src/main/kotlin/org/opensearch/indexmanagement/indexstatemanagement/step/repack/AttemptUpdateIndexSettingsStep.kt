...
class AttemptUpdateIndexSettingsStep(private val action: RepackAction) : Step(NAME) {
    ...
    // 3. create UpdateIndexSettings with SETTING_BLOCKS_WRITE(set readOnly),
    // index.merge.policy.expunge_deletes_allowed=0, and rest of new index settings from job spec
    ...
    override suspend fun execute(): Step {
        ...
        // TODO: do I need to check this settings?
...
//        }
    ...
    private suspend fun getAnySegmentMode(context: StepContext, indexName: String): Tuple<String, String>? {
        ...
        if (modeAttributes.size > 1) {
            logger.warn("Found more than 1 attribute with suffix '.mode': $modeAttributes")
        }

        val selectedModeAttribute = modeAttributes.entries.random()

        logger.info("Select attribute: $selectedModeAttribute")
    ...
    private suspend fun setIndexSettings(indexName: String, context: StepContext): Boolean {
        ...
        try {
            logger.info("Closing index to update codec setting")
            ...
            logger.info("Update index codec setting")
            ...
            logger.info("Opening index after update codec setting")
